...
cfg = dict(G.CONFIG)
cfg["T_outer"] = 100
out_path = OUT / "runs_grassmann_diag.jsonl"
if out_path.exists(): out_path.unlink()

print(f"BCI Grassmann diag (with v_err and true_grad_norm), T_outer={cfg['T_outer']}")
manifold = G.GrassmannManifold(n=22, p=cfg["p"])

t_start = time.time(); n_done = 0
n_total = len(cfg["subjects"]) * len(cfg["seeds"]) * len(REC_ALGOS)

for sid in cfg["subjects"]:
    sd = G.load_subject(f"/mnt/user-data/uploads/{sid}.mat", classes=tuple(cfg["classes"]))
    for seed in cfg["seeds"]:
        Ctr, ytr, Cval, yval = G.split_train_val(sd, cfg["n_tr_per_class"], seed=seed)
        problem = G.Problem(G, train_data=(Ctr, ytr), val_data=(Cval, yval), d=cfg["d"])
        for algo_name, fn in REC_ALGOS.items():
            t0 = time.time()
            log = run_one_diag(algo_name, fn, problem, manifold, label=sid, seed=seed, cfg=cfg)
