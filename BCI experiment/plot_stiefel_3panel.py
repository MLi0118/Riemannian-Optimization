"""
3-panel figure (a, c, d) for Stiefel:
...
runs = [json.loads(l) for l in open(RESULTS / "runs_stiefel_diag3.jsonl")]
...
plot_panel(axes[1], runs, "true_grad_norm",
           r"$\|\mathrm{grad}_W F\|_{\mathrm{St}}$",
...
fig.suptitle("BCI IV-2a binary motor-imagery, PL regime (T = 100, 9 subjects × 3 seeds)",
             fontsize=12, y=1.02)
fig.tight_layout()
fig.savefig(FIGS / "stiefel_3panel.png", dpi=160, bbox_inches="tight")
fig.savefig(FIGS / "stiefel_3panel.pdf", bbox_inches="tight")
plt.close(fig)
print(f"  -> {FIGS}/stiefel_3panel.{{png,pdf}}")
