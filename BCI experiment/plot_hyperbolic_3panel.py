"""
3-panel figure (a, c, d) for synthetic Poincaré-ball regression:
...
def plot_panel(ax, runs, key, ylabel, title, log_y=True, floor=None, clip_lo=None, clip_hi=None):
...
runs = [json.loads(l) for l in open(RESULTS / "runs_hyperbolic_diag.jsonl")]
...
fig.suptitle("Synthetic Poincaré-ball regression, PL regime (T = 100, 5 data × 3 init seeds)",
             fontsize=12, y=1.02)
fig.tight_layout()
fig.savefig(FIGS / "hyperbolic_3panel.png", dpi=160, bbox_inches="tight")
fig.savefig(FIGS / "hyperbolic_3panel.pdf", bbox_inches="tight")
plt.close(fig)
print(f"  -> {FIGS}/hyperbolic_3panel.{{png,pdf}}")
